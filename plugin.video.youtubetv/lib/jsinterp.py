"""A JavaScript interpreter, vendored from yt-dlp.

YouTube signs every media URL with an n parameter that the page rewrites
before fetching, using a function generated afresh in each player release --
the nsig challenge. Unlike the signature transform in cipher.py, which is three
primitives whose order the player merely selects, nsig is real code: loops,
closures, arrays of helpers, charCodeAt arithmetic, different every week. There
is no algorithm to reimplement, only a language to run.

So this is not our code and is deliberately not adapted. yt-dlp is released
into the public domain under the Unlicense, and its interpreter is the one
implementation kept working against every player Google ships. Vendoring it
verbatim means a fix upstream is a copy rather than a re-derivation.

    Source:  yt_dlp/jsinterp.py and the helpers it imports from yt_dlp/utils
    Version: 2026.8.19
    Licence: Unlicense (public domain) -- see LICENSE-yt-dlp.txt

Only the import line is changed: the helpers it takes from yt_dlp.utils are
gathered in jsutils.py beside this file, so the module stands alone.
"""

class ExtractorError(Exception):
    """Stands in for yt_dlp.utils.ExtractorError.

    The only edit to the vendored file besides its import line: jsinterp raises
    this, and pulling in yt-dlp's version would drag the rest of yt-dlp with it.
    """
