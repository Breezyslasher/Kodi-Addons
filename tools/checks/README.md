...
| `test_pages.py` | the Home and Library readers | four traps at once: the Library's filter/sort grid is a flattened cross product, so pairing selector *i* with content *i* names the wrong row; a scheduled recording that is on the air carries its videoId only inside a popup dialog and was dropped; two airings of one show deduplicated to one because they share a destination; and Home's first `nextContinuationData` belongs to its first shelf, not to the page. Also covers `any_rows`, the reader that needs no container name, which must return the innermost titled renderers and not the page that holds them. Also covers `any_rows`, the reader that needs no container name; the Browse tab, whose five category chips share one browseId and are told apart only by `params`; `action_text`, which reads what a DVR call was answered with out of its `actions` block without letting a button's own label win; and `browse_tabs`, where a network's tabs title themselves with a plain string, the selected tab's own shelf continuation must not be mistaken for the tab's token, and a tab arriving with no `content` at all must still be read by the token or browse endpoint it does carry |
