...
### OAuth browses; no request we can build yet also gets DASH
...
Identical for both titles.

**That is where it stands, not why.** It would be easy to read this as "the
web client does not take OAuth" and stop, and that reading is not supported.
The client survey further up these notes was run on a *cookie* session, and
records `TVHTML5_UNPLUGGED` answering **400 INVALID_ARGUMENT** -- the very
identity that answers 200 on a bearer session. So the pairing is not fixed,
and a 400 here is InnerTube rejecting the *request*, not the credential; the
same section already says it "declines to say which argument it dislikes --
error.details is absent, not merely unread".

So the honest statement is narrower: **no request the addon currently knows
how to build gets both a bearer-accepted session and a DASH manifest.**
...
The unexplored lead is the context. `WEB_UNPLUGGED` + bearer answers
INVALID_ARGUMENT, and a bearer session has no cookie jar, so `_bootstrap`
reads a *signed-out* page and the web context is assembled from whatever that
page carries -- possibly without the `rolloutToken`, `configInfo` and
`visitorData` the real web player sends every time. Whether the web client
refuses the token or refuses an incomplete context has not been told apart.

`probe_oauth_dash` now does that, and only in the case that was going to fail
anyway: once per video, after the ordinary search has come up empty, it asks
`WEB_UNPLUGGED` with the context as built, with the session state stripped,
with `visitorData` alone, and each of those again without the `params` blob
the web player sends -- plus `TVHTML5_UNPLUGGED` with no `params`, in case
that is what selects SABR. It logs the status and the dash/sabr flags for
each. This is option 2 from "The client survey" above, finally being pulled
on.
...
### The context theory is dead: the web client refuses the token itself

`probe_oauth_dash` ran. The result rules out the one explanation that was
left:

```
oauth dash probe: the web context carries visitorData, rolloutToken, configInfo
[WEB_UNPLUGGED / context as built]              400 INVALID_ARGUMENT
[WEB_UNPLUGGED / context without session state] 400 INVALID_ARGUMENT
[WEB_UNPLUGGED / context with visitorData only] 400 INVALID_ARGUMENT
[WEB_UNPLUGGED / no params, context as built]   400 INVALID_ARGUMENT
[WEB_UNPLUGGED / no params, bare context]       400 INVALID_ARGUMENT
[TVHTML5_UNPLUGGED / no params]                 200 dash=False sabr=True
```

The context was never incomplete: `_bootstrap` reads `visitorData`,
`rolloutToken` and `configInfo` off a signed-out page perfectly well, so the
bearer session had all three. `WEB_UNPLUGGED` refused every shape of the
request -- full context, stripped, `visitorData` alone, with and without the
`params` blob. And `params` is not what selects SABR: `TVHTML5_UNPLUGGED`
without it still answers `dash=False sabr=True`.

A refusal that survives every variation of the request is a refusal of the
credential. So, now on evidence rather than inference: **the web client will
not accept a bearer token, and it is the only client served DASH.** OAuth is
a browsing credential on this surface; playback needs the cookie jar. The
"cannot play" dialog says exactly that when the session is a code sign-in,
instead of blaming the stream.

What remains untried is option 2 from "The client survey": `ANDROID_UNPLUGGED`
and `IOS_UNPLUGGED` answer 400 on *both* credentials, so they have never
worked at all and it is unknown what they would be served if they did. Telling
that apart needs a capture from the YouTube TV Android or iOS app rather than
the browser -- the same method that settled the SABR URL.

### The guide's station metadata is under `station`, not `epgStationRenderer`

143 of 150 channels listed as raw ids -- `UCtfoIx_MZ0h5bX3i9YsF5xw` instead of
`WTAE 4` -- with no logos. The diagnostic said where it went:

```
guide: a station row carried no name. Row keys
  ['airings', 'navigationEndpoint', 'station', 'stationId', 'trackingParams'];
  station keys none
guide: 143 of 150 stations came back with no name or logo, so their ids are standing in
```

The row has a `station` key. The parser only knew `epgStationRenderer`, so it
read nothing and fell back to the id. Not a missing field -- a renamed one.

`parse_epg` now takes the row's own `station` node, unwraps a single
`*Renderer` wrapper if there is one, and looks for a name under `name`,
`callSign`, `title`, `primaryText` or `displayName` and a logo under `icon`,
`thumbnail`, `logo` or `avatar`, at the node and then one level in. Both
shapes are covered by tests, along with an id-only row, which is kept with
its airings rather than dropped.

### Our non-web client identities were never verified, and it shows

Asked whether the addon sends the right parameters for the TV platform, the
answer is no, and there is a specific reason to think so:

| client | id | version we send |
|---|---|---|
| `ANDROID_UNPLUGGED` | 29 | 6.36 |
| `IOS_UNPLUGGED` | 33 | 6.36 |
| `TVHTML5_UNPLUGGED` | 65 | 6.36 |

An Android app and an iOS app do not ship the same version string, and a
TVHTML5 client's version is date-shaped -- plugin.video.youtube's working
TVHTML5 client sends `7.20250923.13.00` -- never `6.36`. One number was
copied across three clients.

The contexts are just as thin. Ours for `TVHTML5_UNPLUGGED` is `platform: TV`
and a Cobalt user agent. The one that works in plugin.video.youtube also
carries `deviceMake: Samsung`, `deviceModel: SmartTV`, `osName: Tizen`, an
`osVersion` composed from four parts, `browserName: SamsungBrowser` and
`browserVersion: 9.2`.

Only `WEB_UNPLUGGED` was ever built from a capture, and it is the only one
that has ever answered 200 on cookies. So the three `INVALID_ARGUMENT`
refusals may be exactly what they say -- an argument InnerTube dislikes,
namely a client claiming a version it could not have -- rather than the
surface refusing those clients. That is a lead, not a conclusion, and the
values to replace them with have to come from a capture of the real apps
rather than from being made up here.

### tv.youtube.com serves the web build to every user agent

Run signed in with cookies, so the fetch reached the real page rather than
`/welcome`:

```
ANDROID_UNPLUGGED     we send id=29 v=6.36             | page says WEB_UNPLUGGED 1.20260826.04.00 41 | 89097 bytes
IOS_UNPLUGGED         we send id=33 v=6.36             | page says WEB_UNPLUGGED 1.20260826.04.00 41 | 88986 bytes
TVHTML5_UNPLUGGED     we send id=65 v=6.36             | page says WEB_UNPLUGGED 1.20260826.04.00 41 | 88899 bytes
TV_UNPLUGGED_ANDROID  we send id=63 v=1.37             | page says WEB_UNPLUGGED 1.20260826.04.00 41 | 89136 bytes
TV_UNPLUGGED_CAST     we send id=58 v=0.1              | page says WEB_UNPLUGGED 1.20260826.04.00 41 | 88828 bytes
WEB_UNPLUGGED         we send id=41 v=1.20260826.04.00 | page says WEB_UNPLUGGED 1.20260826.04.00 41 | 88810 bytes
```

A Cobalt agent gets the same build as Firefox. The page sizes differ slightly
between agents, so something is varying, but the client identity it declares
is not. So `tv.youtube.com/` cannot supply a TV client version, and the three
copied `6.36`s stay unverified from this direction.

The probe now also asks `tv.youtube.com/tv`, `www.youtube.com/tv` and
`www.youtube.com/`. The `/tv` shells are what a real television fetches and
they are served without signing in, which also makes them the only part of
this probe that works on a token session. Read honestly, `/tv` is plain
YouTube's `TVHTML5` (client 7) rather than `TVHTML5_UNPLUGGED` (65), so it
settles what a TVHTML5 version looks like and what today's is, not whether
the unplugged variant shares it -- but `6.36` cannot be right either way.

If those come back with the web build too, this whole direction is closed and
the versions can only come from a capture of the real YouTube TV apps.

### The TV shell hands over a real TVHTML5 version

`www.youtube.com/tv`, asked with the Cobalt user agent, is served the actual
television build -- and only that agent is:

```
-- YouTube TV app shell (https://www.youtube.com/tv)
TVHTML5_UNPLUGGED   we send v=6.36 | page says TVHTML5 7.20260826.15.00 id 7 | 185224 bytes
every other agent                  | page says -       -                -    |   5709 bytes
tv.youtube.com/tv                  | HTTP 404 for all six
```

So the Cobalt user agent already in the spec is what unlocks it, and a
TVHTML5 version is `7.<date>.<n>.00`. `6.36` is not a version of that client
at all -- it is the Android unplugged app's number, copied onto three
unrelated clients.

`tv_client_version` now reads it the way `_bootstrap` reads the web client's:
cached for a day, ignored unless the page actually says `TVHTML5`, and
falling back to the old value on any doubt. `context()` and `_headers` use it
for `TVHTML5_UNPLUGGED`.

Read honestly, this is plain YouTube's `TVHTML5` (client 7), not
`TVHTML5_UNPLUGGED` (client 65). Whether the unplugged variant rides the same
version stream is unknown. But a measured version of the right shape beats an
invented one of the wrong shape, and this is the first time that client has
claimed a version it could plausibly have.

Two other things the probe turned up, for the record: `www.youtube.com` gives
`MWEB 2.20260827.01.00` (id 2) to the mobile agents and `WEB
2.20260826.01.00` (id 1) to Cobalt, so those agents are recognised as what
they claim; and `tv.youtube.com/tv` does not exist. `ANDROID_UNPLUGGED` and
`IOS_UNPLUGGED` still claim `6.36` because nothing found so far states an app
version -- that needs a capture of the apps themselves.

### The real TVHTML5 version does not unlock the TV client on cookies

The survey was re-run with `TVHTML5_UNPLUGGED` claiming `7.20260826.15.00`
read from the TV shell, signed in with cookies:

```
tv bootstrap: the TV shell states TVHTML5 7.20260826.15.00
player -> HTTP 400 as TVHTML5_UNPLUGGED   INVALID_ARGUMENT
player -> HTTP 400 as ANDROID_UNPLUGGED   INVALID_ARGUMENT
player -> HTTP 400 as IOS_UNPLUGGED       INVALID_ARGUMENT
player -> HTTP 403 as TV_UNPLUGGED_ANDROID PERMISSION_DENIED
player -> HTTP 404 as TV_UNPLUGGED_CAST    NOT_FOUND
client WEB_UNPLUGGED  formats=26 url=0 cipher=26 dash=True sabr=True drm=True
```

So the copied version was not what those three were objecting to. It was
still worth replacing -- `6.36` was not a version that client could have --
but it is not the answer, and the 400s remain unexplained.

The 403 is worth repeating: the session probe confirms the same cookies are
served a signed-in page at that moment, so `TV_UNPLUGGED_ANDROID` is refused
the surface rather than the session.

One trap to note. That run logged `as TVHTML5_UNPLUGGED v6.36` **while
sending 7.20260826.15.00**: the log line read the static spec, and only
`context()` and `_headers` read the fetched value. A result that appears to
disprove the change being tested is worse than no result, so
`effective_version` is now the single place that decides, and all three read
it.
