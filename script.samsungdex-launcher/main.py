...
Streams the Samsung DeX desktop (or a scrcpy virtual desktop) to Kodi.
Inspired by TuxDex (https://github.com/semarainc/TuxDex), with several
improvements:
  - No miraclecast, no root, no NetworkManager changes
  - Mirrors the DeX display directly with scrcpy --display-id
  - Auto-detects the DeX display id via scrcpy --list-displays
  - Virtual desktop mode via scrcpy 3.x --new-display (no DeX session
    required - desktop-style windowing on One UI 7 / Android 15+)
  - Audio through scrcpy itself (no ffplay/RTP sidecar)
  - LibreELEC and Flatpak Kodi support inherited from scrcpy Launcher

Binaries (adb / scrcpy) are shared with script.scrcpy-launcher, which is
...
ADDON = xbmcaddon.Addon(id='script.samsungdex-launcher')
ADDON_PATH = ADDON.getAddonInfo('path')
ADDON_NAME = ADDON.getAddonInfo('name')

ARCH = platform.machine()
LIB_DIR = 'lib_arm64' if ARCH == 'aarch64' else 'lib_x64'
...
def get_bin_path():
    """Binaries live in the scrcpy Launcher addon (declared dependency)."""
    scrcpy_addon = xbmcaddon.Addon('script.scrcpy-launcher')
    return os.path.join(scrcpy_addon.getAddonInfo('path'), 'bin', LIB_DIR)
...
def prepare_binaries():
    try:
        ...
        log(f"chmod failed (may already be executable): {e}")


def get_common_args():
    """scrcpy args shared by DeX and virtual desktop modes."""
    args = []
...
def build_scrcpy_cmd(mode_args, serial=None):
    cmd = [SCRCPY_PATH]
    render = get_render()
    if render:
    ...
    cmd.extend(get_common_args())
    return cmd


def launch_libreelec(cmd, ip=None, port=None):
    """LibreELEC: stop Kodi, run scrcpy via systemd, restart Kodi after."""
    scrcpy_cmd = ' '.join(cmd)
    log(f"LibreELEC cmd: {scrcpy_cmd}")

    connect_line = f'{ADB_PATH} connect {ip}:{port} >> $LOG 2>&1\nsleep 1\n' if ip else ''

    launcher = "/storage/.kodi/temp/dex_launch.sh"
    script = f'''#!/bin/sh
LOG="/storage/.kodi/temp/dex.log"
echo "=== DEX START ===" > $LOG
date >> $LOG
...
def launch(mode, use_wifi):
    ...
    # adb prep + detection happen while Kodi is still running (works on
    # LibreELEC too: listing displays does not open a window).
    ...
    cmd = build_scrcpy_cmd(mode_args, serial)

    if IS_LIBREELEC:
        launch_libreelec(cmd, ip, port)
    else:
...
def main():
    ...
    if not os.path.exists(SCRCPY_PATH):
        xbmcgui.Dialog().ok("Error", "scrcpy binaries not found.\n"
                            "Install/repair the 'scrcpy Launcher' addon (dependency).")
        return
    ...
    if sel == 0:
        launch('dex', use_wifi=False)
    elif sel == 1:
        launch('dex', use_wifi=True)
    elif sel == 2:
        launch('virtual', use_wifi=False)
    elif sel == 3:
        launch('virtual', use_wifi=True)
    elif sel == 4:
